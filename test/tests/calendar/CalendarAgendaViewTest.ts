...
o.spec("CalendarAgendaViewTest", function () {
    o("earliestEventToShowTimeIndicator", () => {
        ...
        o(earliestEventToShowTimeIndicator(events.map((e) => makeEventWrapper(e)), allDayStartDate)).equals(1)("lower index preferred")
        o(earliestEventToShowTimeIndicator(events.map((e) => makeEventWrapper(e)), actualEventStart)).equals(3)("if the date is equal, skip the event")
        o(earliestEventToShowTimeIndicator(events.map((e) => makeEventWrapper(e)), nextEventStart)).equals(null)("no dates left")
