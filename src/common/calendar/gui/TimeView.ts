...
export class TimeView implements Component<TimeViewAttributes> {
	...
	private buildGridData(allColumns: Array<ColumnData>): Map<Id, GridEventData> {
		...
		const updatedColumns = new Array<ColumnData>(allColumns.length)
		...
		for (const [columnIndex, columnData] of allColumns.entries()) {
			for (const [eventId, eventRowData] of columnData.events.entries()) {
				...
				// Fallback for last elements who can't expand and by consequence have size <= 0
				if (spanSize <= 0) {
					spanSize = 1
				}

				const zeroIndexedStart = gridColumnStart - 1
				if (updatedColumns[zeroIndexedStart] == null) {
					updatedColumns[zeroIndexedStart] = {
						events: new Map([[eventId, eventRowData]]),
						lastEventEndingRow: eventRowData.end,
					}
				} else {
					updatedColumns[zeroIndexedStart].events.set(eventId, eventRowData)
					updatedColumns[zeroIndexedStart].lastEventEndingRow = eventRowData.end
				}
		...
		this.applyRetroactiveShifts(gridData, allColumns, updatedColumns)
	 ...
	 * @private
	 */
	private applyRetroactiveShifts(gridData: Map<Id, GridEventData>, allColumns: Array<ColumnData>, updatedColums: Array<ColumnData>): void {
		console.log("All Data", allColumns)
		...
		for (const [eventId, gridInfo] of gridData.entries()) {
			...
			// We are over the maximum column count, we should move back
			if (gridInfo.column.start + gridInfo.column.span >= updatedColums.length) {
				console.log("Bounds =>", gridInfo, gridInfo.column.start + gridInfo.column.span >= allColumns.length, allColumns.length, gridData.size)
				let newStart = 0
				while (
					newStart < updatedColums.length &&
					this.findFirstOverlapFast(gridInfo.row, updatedColums[newStart].events) &&
					newStart <= gridInfo.column.start
				) {
					newStart++
				}

				if (newStart < gridInfo.column.start) {
					gridData.set(eventId, {
						...gridInfo,
						column: {
							...gridInfo.column,
	...
	private updateBlockerShifts(eventId: Id, size: number, blockerShift: Map<Id, number>): void {
		console.log("BlockerShifts ->", { eventId, size })
		...
		for (const blockerGroup of blockerGroups) {
			for (const [blocker] of blockerGroup.entries()) {
				...
				console.log("Blocker ->", { eventId, blocker, size, newShift: blockerShiftInfo + size - 1 })
	...
	private renderCells(timeScale: TimeScale, timeRange: TimeRange): Children {
		...
		for (let hourIndex = 0; hourIndex < numberOfIntervals; hourIndex++) {
			const rowBoundStart = hourIndex * 12 + 1
			const rowBoundEnd = rowBoundStart + 12
			children.push(
				m(".z1.w-full", {
					style: {
						gridRow: `${rowBoundStart} / span ${12}`,
